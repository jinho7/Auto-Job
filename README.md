...
Notion 공고 페이지(또는 지원 페이지 주소)로 지원서를 열고, 자기소개서 전까지의 인적사항을 AI 가 채웁니다. **제출은 하지 않습니다.**

```bash
autojob apply "https://www.notion.so/…공고 페이지…"
```
...
4. **리포트:** 입력 요약, 비워둔 값, 참고사항, 막힌 동작, 화면 캡처를 `data/runs/…_apply-…/` 에 남기고 알림을 보냅니다. 브라우저 창은 검토할 수 있게 그대로 둡니다.
